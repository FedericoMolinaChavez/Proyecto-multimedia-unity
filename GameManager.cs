...
public class GameManager : MonoBehaviour {
	...
	void checkCameraCollider(){
        RaycastHit hit;
		GameObject.Find("FreeCharacter_model").layer = 0;
		Vector3 Chaser = GameObject.Find("FirstPersonCharacter2").transform.position;
		Vector3 Hidden = GameObject.Find("FreeCharacter_model").transform.position;
		Vector3 forward = GameObject.Find("FirstPersonCharacter2").transform.forward;
		Vector3 Direction = Hidden - Chaser;
		Direction[1] = 0;
		if(Physics.Raycast(Chaser,Direction,out hit) && Vector3.Angle(Direction,forward) < 55f * 0.5f){
			if(hit.collider.gameObject.name == "FreeCharacter_model"){
				encontrado = true;
			}
		}
	}
	...
	void Update () {
		//---------------------------------------------------------------Conteo		
	  	if(countOver == false){	
	        ...
	        if(timeLeft < 0)
	        {
	            GameObject.Find("player2").GetComponent<FPC>().enabled = true;	//Habilitar control 
	            countOver = true;          
	        }else{
    ...
    {
        if(countOver == true && col.gameObject.name == "FreeCharacter_model")
        {
        	//Time.timeScale = 0;
        	GameObject.Find("winText").GetComponent<Text>().text = "Ganador: Jugador 1";
        	GameObject.Find("Ganador").GetComponent<Canvas>().enabled = true;
        }else if(countOver == true && encontrado == true && col.gameObject.name == "Lumberjack3"){
        	//Time.timeScale = 0;
